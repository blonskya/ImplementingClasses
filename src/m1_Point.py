...
class Point(object):
    ...
    def move_to(self, ex, why):
        self.totmve +=  math.sqrt((self.x-ex)**2 + (self.y - why)**2)
        self.x = ex
        self.y = why
        self.moves += 1

    def move_by(self, ex, why):
        self.totmve += math.sqrt(ex**2 + why**2)
        self.x += ex
        self.y += why
        self.moves += 1
    ...
    def get_distance_from(self,p2):
        dst = math.sqrt((self.x-p2.x)**2 + (self.y - p2.y)**2)
        return dst
    def get_distance_from_start(self):
        dist = math.sqrt((self.x-self.origx)**2 + (self.y - self.origy)**2)
        return dist
    def get_distance_traveled(self):
        return self.totmve
    def closer_to(self,pone,ptwo):
        if math.sqrt((self.x-pone.x)**2 + (self.y - pone.y)**2) > math.sqrt((self.x-ptwo.x)**2 + (self.y - ptwo.y)**2):
            return ptwo
        else:
            return pone
    def halfway_to(self,pother):
        pnew=Point((self.x+pother.x)/2,(self.y+pother.y)/2)
        return pnew
